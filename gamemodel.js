 class GameModel {
    ...
    //function to move down tetrominoes
    moveDown() {
        if (this.fallingPiece === null) {
    ...
    //function to move tetrominoes sideways(left and right)
    move(side) {
        if (this.fallingPiece === null) {
            return
        }
        ...
        if (side === "right") {
            // to move tetrominoes on right side
            if (!this.collision(x + 1, y)) {
                this.fallingPiece.x += 1
            }
        } else {
            //to move tetrominoes on left side
            if (!this.collision(x - 1, y)) {
                this.fallingPiece.x -= 1
            }
        }
    ...
    //to rotate tetrominoes for different shape
    rotate() {
        if (this.fallingPiece !== null) {
